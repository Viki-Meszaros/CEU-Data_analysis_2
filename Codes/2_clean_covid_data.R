...
# Covid data
cv <- read_csv(paste0(my_path,'raw/covid_10_26_2020_raw.csv'))
# Population data
...
for ( i in seq_along( countries_nm ) ){
  ...
  if ( length( c_partial ) == 2 & sum( is.na( c_partial ) ) == 1 ){
    ...
    df <- df %>% filter( !(log_select & is.na( df$cases ) ) )
  }
}
...
df <- df %>% filter( !( is.na( population ) | is.na( cases ) | is.na( death ) ))


#####
# Save clean data

write_csv( df , paste0(my_path,'Clean/covid_pop_10_26_2020_clean.csv'))
